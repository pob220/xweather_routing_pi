...
spec = importlib.util.spec_from_file_location(
    "recovery", Path(__file__).parents[1] / "collect-reviewed-1.17.4.py")
...
class ReviewedRecovery(unittest.TestCase):
    ...
    def test_replacement_requires_exact_successful_job(self):
        job = {'name': 'debian13-x86_64-recovery', 'status': 'success', 'job_number': 123}
        self.assertEqual(recovery.replacement_trixie([job], 'a' * 40, 'wf'),
                         (123, 'a' * 40, 'wf'))
        for jobs in ([], [job, job], [dict(job, status='failed')],
                     [dict(job, name='unrelated-build')]):
            with self.assertRaises(ValueError):
                recovery.replacement_trixie(jobs, 'a' * 40, 'wf')

    def test_replacement_rejects_missing_provenance(self):
        job = {'name': 'debian13-x86_64-recovery', 'status': 'success', 'job_number': 123}
        for revision, workflow in (('main', 'wf'), ('a' * 40, '')):
            with self.assertRaises(ValueError):
                recovery.replacement_trixie([job], revision, workflow)

    def test_wait_ignores_only_explicitly_replaced_trixie(self):
        from unittest.mock import patch
        jobs = [{'job_number': n, 'status': 'failed' if target == 'trixie' else 'success'}
                for target, n in recovery.JOBS.items()]
        retained = {t: n for t, n in recovery.JOBS.items() if t != 'trixie'}
