#!/usr/bin/env python3
"""Recover only the nine reviewed 1.17.4 CI packages; never build or upload.
...
WORKFLOW = "e028b273-3360-4a51-8611-600b86c086dc"
REVISION = "01877da7e4c0c7d414ef29ced0c443e00bab756a"
JOBS = {
    "trixie": 407, "bookworm": 402, "jammy": 405, "noble": 399,
    "bookworm-arm64": 404, "flatpak-x86_64": 406,
    "flatpak-aarch64": 400, "windows-x86": 403, "macos-arm64": 401,
}
...
def replacement_trixie(jobs, revision, workflow):
    if not re.fullmatch(r"[0-9a-f]{40}", revision) or not workflow:
        raise ValueError("Missing immutable recovery revision/workflow")
    matches = [j for j in jobs if j.get("name") == "debian13-x86_64-recovery"]
    if len(matches) != 1 or matches[0].get("status") != "success":
        raise ValueError("Expected one successful Debian 13 recovery job")
...
def collect(output, timeout, recover_trixie=False):
    ...
    if recover_trixie:
        ...
        data = get_json(f"https://circleci.com/api/v2/workflow/{workflow}/job")
        sources["trixie"] = replacement_trixie(data["items"], revision, workflow)
        del retained["trixie"]
    ...
    for target, (number, revision, workflow) in sources.items():
        ...
        if version != "1.17.4.0":
            raise ValueError("Expected version 1.17.4.0")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--current-workflow-trixie", action="store_true")
    args = parser.parse_args()
    collect(args.output, args.wait_seconds, args.current_workflow_trixie)
